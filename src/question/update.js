...
export default (app) => {
  app.post(
    ...
    asyncReq(async (req, res) => {
      ...
      // validate date
      if (
        expirationDate && !moment(expirationDate, moment.ISO_8601).isValid()
      ) {
      ...
      if (req.user.id !== question.owner) {
        res
          .status(403)
          .send({ error: 'Not enough rights to change the question!' })
        return
      }

      // check if data is actually changed
      const textChanged = text && question.text !== text
      const expDateChanged =
        expirationDate &&
        !moment(expirationDate).isSame(question.expirationDate)
      // if not - just send OK
