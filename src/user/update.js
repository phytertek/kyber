...
export default (app) => {
  app.post('/api/user/:id', passport.authenticate('jwt', {session: false}), asyncReq(async (req, res) => {
    const {username, password, password2} = req.body

    let own = req.params.id === 'me'
    own = own ? own : req.params.id === req.user.id
    ...
    const user = await User.get(req.params.id === 'me' ? req.user.id : req.params.id)
